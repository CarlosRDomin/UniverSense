...
def generate_video_from_images(img_folder_or_filelist, video_folder="videos", video_filename="", video_format="mp4", video_fps=15, img_filelist_prefix=""):
	...
	if len(files) > 0:
		...
		for i, f in enumerate(files):
			...
			print "\n{} out of {} frames ({:6.2f}%) written! Time elapsed: {:.2f}s".format(i+1, len(files), 100.0*(i+1)/len(files), (datetime.now()-t_start).total_seconds())
...
if __name__ == '__main__':
	generate_video_from_images(os.path.join("img-ns", "2017-10-02 11-45-57"), video_fps=20, img_filelist_prefix="out_")
