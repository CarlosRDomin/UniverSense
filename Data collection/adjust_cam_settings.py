...
from collect_data import Spotter, FakeWorkerDrone, FakeVideoCapture
...
class CamFrameGrabberThread(QThread):
	...
	def __init__(self):
		...
		self.dev_selected_serial_num = self.DEFAULT_CAM_SERIAL_NUM
		self.cap = None
	...
	def change_selected_device(self, dev_selected):
		...
		self.dev_selected_name, self.dev_selected_vend_id, self.dev_selected_prod_id, self.dev_selected_serial_num = UvcCapture.parse_human_friendly_identifier(dev_selected)
...
# def cam_info_to_str(cam_name, cam_vend_id, cam_prod_id, cam_serial_num):
# 	"""
...
# 	:param cam_serial_num: Camera serial number, as provided by the uvc library
# 	:return: String containing the name, vendor id, product id and serial number of the camera in a human-friendly format.
# 	"""
...
def on_lblCamImg_mouseEvent(event):  # Keep track of cursor location to display pixel information on the UI, as well as to print such info in the console on click
	...
	# If out of bounds, don't update cv_evaluate_px_at (evaluating out of bounds would cause an error)
...
def repaint_cam_img():
	...
	# window.lblCamImg.update()  # Not necessary to update or repaint anymore, Qt will do that for us automatically
