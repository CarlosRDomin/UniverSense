...
def take_calibration_images(cam_settings=None):
	...
	"""
	from collect_data import Spotter  # Don't require collect_data unless they need to capture calibration images
	cv = Spotter()  # Create a Spotter object so we use the same USB cam as the one the Spotter will use
	if cam_settings is not None: cv.CAMERA_SETTINGS_FILE = cam_settings
