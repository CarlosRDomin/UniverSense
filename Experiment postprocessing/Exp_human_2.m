...
data = loadRealExperimentData(struct('datetime',{'2017-10-11 20-18-10'}, 'ch','80'), [], 2, 13, 10);
exp = readNPZ(['/Users/shijiapan/Google Drive/log/80/2017-10-11 20-18-10/log_experiment_running_80_2017-10-11_20-18-10.npz']);
...
for strAxCell = {'Y', 'Z'}
    ...
    figure;
    for eventID = 1:9%10%length(expStart)
        eventID
...
%% 
data = loadRealExperimentData(struct('datetime',{'2017-10-11 20-25-09'}, 'ch','80'), [], 2, 13, 10);
exp = readNPZ(['/Users/shijiapan/Google Drive/log/80/2017-10-11 20-25-09/log_experiment_running_80_2017-10-11_20-25-09.npz']);
expStart = exp.tFloat(exp.measured == 1);
expStop = exp.tFloat(exp.measured == 0);
expStop(1:2) = [];
cameraIdxOffset = 0.08;
eventPadding = 0;
UAVgravityFactor = 8;%9.81;
axisCount = 0;
...
for strAxCell = {'Y', 'Z'}
    ...
    figure;
    for eventID = 10%:9%10%length(expStart)
        eventID
        eventIdx = find(data.a_UAV.(strAx).tFloat >= expStart(eventID-9)-eventPadding & data.a_UAV.(strAx).tFloat < expStop(eventID-9)+eventPadding);
        uavSig{axisCount, eventID} = [data.a_UAV.(strAx).tFloat(eventIdx), axisYP.*(UAVgravityFactor.*(data.a_UAV.(strAx).measured(eventIdx)-uavOffset))];
        eventIdx2 = find(data.a_cam.(strAx).tFloat+cameraIdxOffset >= expStart(eventID-9)-eventPadding & data.a_cam.(strAx).tFloat+cameraIdxOffset < expStop(eventID-9)+eventPadding);
        camSig{axisCount, eventID} = [data.a_cam.(strAx).tFloat(eventIdx2)+cameraIdxOffset, (data.a_cam.(strAx).measured(eventIdx2)'-camOffset)];
        plot(uavSig{axisCount, eventID}(:,1),uavSig{axisCount, eventID}(:,2));hold on;
        plot(camSig{axisCount, eventID}(:,1),camSig{axisCount, eventID}(:,2));hold on;
...
matchingPrint
similarity
save('human_2.mat','uavSig','camSig','fingerprintUAV','fingerprintCAM');
