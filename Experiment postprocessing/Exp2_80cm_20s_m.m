...
data = loadRealExperimentData(struct('datetime',{'2017-10-05 17-28-07'}, 'ch','80'), [], 2, 13, 10);
exp = readNPZ(['/Users/shijiapan/Google Drive/log/80/2017-10-05 17-28-07/log_experiment_running_80_2017-10-05_17-28-07.npz']);
expStart = exp.tFloat(exp.measured == 1);
expStop = exp.tFloat(exp.measured == 0);
expStop(1:2) = [];
cameraIdxOffset = 0;
eventPadding = 0;
UAVgravityFactor = 7;%9.81;
axisCount = 0;
...
for strAxCell = {'Y', 'Z'}
	...
	strAx = strAxCell{:};
	figure; plot(data.a_UAV_orig.(strAx).tFloat, axisYP.*UAVgravityFactor.*data.a_UAV_orig.(strAx).measured, 'g', 'LineWidth',2); hold on; 
    plot(data.a_UAV.(strAx).tFloat, axisYP.*UAVgravityFactor.*(data.a_UAV.(strAx).measured), 'r', 'LineWidth',2); 
...
matchingPrint
save('mid2_80cm_20s.mat','uavSig','camSig','fingerprintUAV','fingerprintCAM');
